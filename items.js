...
const CORPSE_TEMPLATE_MAP = {
    ...
    elena: 'elena_corpse',
    aisha: 'aisha_corpse',
    mine_supervisor: 'corpse_supervisor',    // 监工
    mad_miner: 'corpse_miner',              // 发狂矿工（通用矿工尸体）
    mad_supervisor: 'corpse_supervisor'     // 发狂监工（通用监工尸体）
};

// 根据尸体物品ID查找对应的尸体模板
// 按关键词长度降序排列，优先匹配最长（最具体）的名称
// 例如 "mine_supervisor" 应优先于 "mine" 匹配，"mad_miner" 应优先于 "miner" 匹配
function getCorpseTemplateByItemId(itemId) {
    const sortedKeys = Object.keys(CORPSE_TEMPLATE_MAP).sort((a, b) => b.length - a.length);
    for (const key of sortedKeys) {
        if (itemId.includes(key)) {
            return ITEM_TEMPLATES[CORPSE_TEMPLATE_MAP[key]] || null;
        }
    }
    return null;
}
...
function createCorpse(npcId, drops = [], extraProps = {}) {
    ...
    if (!template) {
        let chineseName = `${npcId}的尸体`;
        if (typeof CHARACTER_TEMPLATES !== 'undefined' && CHARACTER_TEMPLATES[npcId]) {
            chineseName = `${CHARACTER_TEMPLATES[npcId].name}的尸体`;
        }
        return {
            id: `corpse_${npcId}_${Date.now()}`,
            name: extraProps.name || chineseName,
            type: "misc",
            desc: extraProps.desc || "一具倒在血泊中的尸体，可以搜刮。",
            loot: [...drops],
            dismemberable: false,
            usable: false,
            customAction: false
    ...
    // 使用模板创建尸体，drops 覆盖模板中可能存在的 loot
    const isDismemberable = template.dismemberable || false;
    return {
        id: `corpse_${npcId}_${Date.now()}`,
        name: extraProps.name || template.name,
        type: "misc",
        desc: extraProps.desc || template.desc,
        loot: drops.length > 0 ? [...drops] : (template.loot ? [...template.loot] : []),
        dismemberable: isDismemberable,
        usable: true,
        customAction: true,
